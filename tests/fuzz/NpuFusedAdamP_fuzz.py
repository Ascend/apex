...
with atheris.instrument_imports():
    from apex.optimizers import NpuFusedAdamP

@atheris.instrument_func
def Test_NpuFusedAdamP(input_bytes):

    input_list = [True, False]
    check_list_input = input_bytes.decode('utf-8', 'ignore').strip().split(',')
    if not check_list_input or len(check_list_input) != 9:
        return False
    try:
        for i in range(9):
    ...
    delta = check_list_input[6]
    wd_ratio = check_list_input[7]
    number_of_params = int(check_list_input[8])
    nesterov = input_list[random.randint(0, 1)]
    ...
    try:
        ...
        NpuFusedAdamP(params, lr, betas, eps, weight_decay, delta, wd_ratio, nesterov)
    except Exception as e:
        print(e)
        return True

if __name__ == "__main__":
    atheris.Setup(sys.argv, Test_NpuFusedAdamP)
    atheris.Fuzz()
